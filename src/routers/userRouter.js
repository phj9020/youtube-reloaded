...
import {privateMiddleware, publicOnlyMiddleware, multerMiddleware} from '../middlewares';

const userRouter = express.Router();

userRouter.get("/logout", privateMiddleware, logout);
userRouter.route("/edit").all(privateMiddleware).get(getEdit).post(multerMiddleware.single('avatar'), postEdit);
